...
function summarizeCapabilityStatus(state: StatusEntry['state'], detail: string): AccessCheckSummary['status'] {
  ...
  if (['missing', 'not_configured', 'not_logged_in', 'not_running'].includes(state)) {
    return 'missing';
  }
  if (state === 'warning') {
    return 'warning';
...
function buildWarnings({
  ...
  if (gateway.state !== 'running') {
    warnings.push({
      ...
      detail: 'Hermes cannot deliver on connected messaging surfaces until the gateway comes back.'
    });
  }
    ...
    warnings.push({
      ...
      detail:
        [
          ...
          .filter(Boolean)
          .join(' · ') || 'Recent cron signals suggest one or more jobs need a closer look.'
    });
  }
...
function buildRuntimeHealthItems({
  ...
  return [
    ...
    },
    {
      ...
      detail: doctor.issueCount > 0 ? 'Diagnostics found issues to review.' : 'No issues found.',
    ...
    },
    {
      ...
      tone: update.status === 'behind' ? 'warning' : update.status === 'up_to_date' ? 'healthy' : 'default'
    }
  ];
}
  ...
  return Array.from(names)
    ...
    .map((platform) => {
      ...
      const configured = statusEntry
        ...
        : (channels.platforms[platform]?.total ?? 0) > 0 || config.configuredPlatforms.includes(platform);
...
function buildRuntimeProfile(config: ConfigPostureSummary, envEntries: Map<string, string>): RuntimeProfileItem[] {
  ...
  return [
    ...
    {
      label: 'security',
      value: config.tirithEnabled ? 'tirith on' : config.tirithEnabled === false ? 'tirith off' : 'unknown',
      detail:
...
export function composeRuntimeOverview({
  ...
  const connectedPlatforms = Array.from(new Set([...channels.connectedPlatforms, ...gateway.connectedPlatforms])).sort(
  ...
  return {
    ...
    access: {
      authProviders: buildAuthChecks(
        safeStatus.authProviders.length > 0 ? safeStatus.authProviders : safeDoctor.authProviders
      ),
