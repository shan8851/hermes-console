    ...
    <div className="space-y-8">
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {[
          ...
          {
            label: 'completed',
            value: job.repeatCompleted == null ? '—' : String(job.repeatCompleted),
            detail: job.scheduleKind ?? 'Schedule type unknown'
          },
        ...
        <section className="rounded-lg border border-border bg-surface/70 p-4">
          ...
          {detail.hasOutputs ? (
            <div className="mt-4 space-y-3 xl:max-h-[56rem] xl:overflow-auto xl:pr-1">
              {detail.outputs.map((output) => (
                <article key={output.id} className="rounded-md border border-border/70 bg-bg/40 p-3">
                  ...
                  <pre className="mt-3 whitespace-pre-wrap break-words text-sm leading-6 text-fg">
            ...
            <div className="mt-4 rounded-md border border-dashed border-border/80 p-4 text-sm leading-6 text-fg-muted">
              No outputs recorded for this job.
            </div>
          )}
        </section>
      </div>
