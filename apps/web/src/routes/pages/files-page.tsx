...
export const FilesPage = ({
  ...
  const query = useSuspenseQuery(filesQueryOptions());
  ...
  const { keyFiles, memory } = query.data.data;
  const selectedFileEnvelope = selectedFileQuery.data ?? null;
  const selectedFile = selectedFileEnvelope?.data.file ?? null;
  const effectiveSelectedFileError =
    selectedFileEnvelope == null
      ? selectedFileQuery.error?.message ?? selectedFileError
      : null;
  ...
  const summaryItems = [
    {
      ...
      detail:
        "High-signal files across the Hermes root and bounded workspace scope.",
      tone: "default" as const,
    },
    {
      label: "hermes root",
      value: String(
        keyFiles.files.filter((file) => file.scope === "hermes_root").length,
      ),
    ...
    },
    {
      ...
      value: String(
        keyFiles.files.filter((file) => file.scope === "workspace_root").length,
      ),
    ...
    <div className="space-y-8">
      ...
      <section className="max-w-3xl">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <RefreshButton
            ...
            queryKeys={[
              ...
              ...(selectedFileId == null
                ? []
                : [apiQueryKeys.fileContent(selectedFileId)]),
            ]}
          />
        </div>
        ...
        <p className="mt-3 text-sm leading-7 text-fg-muted">
          The files most likely to shape how Hermes behaves: runtime config under
          the Hermes root plus a tight set of useful workspace instructions and
          markdown.
        </p>
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,0.82fr)_minmax(0,1.38fr)]">
        <KeyFilesIndex
          files={keyFiles.files}
          memory={memory}
          selectedFileId={selectedFile?.id ?? selectedFileId}
        />
        <KeyFilePreview
          memory={memory}
          selectedFile={selectedFileEnvelope}
          selectedFileId={selectedFileId}
          selectedFileError={effectiveSelectedFileError}
        />
      </div>
    </div>
  );
