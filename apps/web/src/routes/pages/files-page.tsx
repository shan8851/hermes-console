...
export const FilesPage = ({
  ...
  const effectiveSelectedFileError =
    selectedFileEnvelope == null ? (selectedFileQuery.error?.message ?? selectedFileError) : null;
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="max-w-3xl">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <RefreshButton
            ...
            queryKeys={[
              apiQueryKeys.files,
              ...(selectedFileId == null ? [] : [apiQueryKeys.fileContent(selectedFileId)])
            ]}
          />
        </div>
        ...
        <p className="mt-3 text-sm leading-7 text-fg-muted">
          The files most likely to shape how Hermes behaves: runtime config under the Hermes root plus a tight set of
          useful workspace instructions and markdown.
        </p>
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,0.82fr)_minmax(0,1.38fr)]">
        <KeyFilesIndex files={keyFiles.files} memory={memory} selectedFileId={selectedFile?.id ?? selectedFileId} />
        <KeyFilePreview
          memory={memory}
