...
const getOverallPressureLevel = (memory: MemoryReadResult) => {
  if (
...
export const MemoryPage = () => {
  ...
  const summaryItems = [
    ...
    },
    {
      label: "saved blocks",
      value: String(
        memory.files.memory.entries.length + memory.files.user.entries.length,
      ),
    ...
    <div className="space-y-8">
      ...
      <section className="max-w-3xl">
        ...
        <p className="mt-3 text-sm leading-7 text-fg-muted">
          Hermes stores durable memory in plain markdown files. This page shows
          current usage against configured limits and the saved blocks parsed out
          of those files.
        </p>
      </section>
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <MemoryFilePanel
          file={memory.files.memory}
          limitSource={memory.limits.memory.source}
        />
        <MemoryFilePanel
          file={memory.files.user}
          limitSource={memory.limits.user.source}
        />
      </div>
    </div>
  );
