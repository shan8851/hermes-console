...
describe('readRuntimeOverviewQuery', () => {
  ...
  it('surfaces unreadable runtime files as unreadable_path instead of missing_path', () => {
    const hermesRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'hermes-console-runtime-'));
    ...
    const unreadableConfigIssue = result.issues.find((issue) => issue.id === 'runtime-config-unreadable');
    const missingConfigIssue = result.issues.find((issue) => issue.id === 'runtime-config-missing');
