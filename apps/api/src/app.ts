...
import {
  readKeyFileContentQuery,
  readKeyFilesDataQuery,
} from "@/features/key-files/query-key-files";
...
const createLiveDiagnosticsResponse = async (): Promise<DiagnosticsResponse> => {
  ...
  const capturedAt =
    diagnostics.status.capturedAt ?? diagnostics.doctor.capturedAt ?? null;
...
export const createApp = ({
  ...
  app.get("/api/runtime/diagnostics", async (context) =>
    context.json(await createLiveDiagnosticsResponse()),
  );
  ...
  app.get("/api/skills/:skillId/files/:fileId", (context) => {
    ...
    if (linkedFile == null) {
      return notFound(
        `Linked skill file not found for ${context.req.param("skillId")}/${context.req.param("fileId")}`,
      );
    }
  ...
  app.get("/api/cron/:agentId/:jobId", (context) => {
    ...
    if (detailResult == null) {
      return notFound(
        `Cron detail not found for ${context.req.param("agentId")}/${context.req.param("jobId")}`,
      );
    }
