...
import type { AgentStateSessionRecord } from "@/features/sessions/types";
import type { CronAgentRef, CronObservedRunRecord, HermesCronIndex } from "@/features/cron/types";
  ...
  const jobs = agents
    .flatMap((agent) => {
      ...
      return normalizeCronJobs({
        ...
        runsByJobId: buildObservedRunsByJobId(readStateDbSessions(agent.rootPath)),
      });
    })
