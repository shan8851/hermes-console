...
import type { KeyFileSummary } from "@/features/key-files/types";
import { MemoryPressureBadge } from "@/features/memory/components/memory-pressure-badge";
import type { MemoryReadResult } from "@/features/memory/types";
            ...
            {group.files.length > 0 ? (
              group.files.map((file) => {
                ...
                return (
                  <Link
                    key={file.id}
                    href={`/files?file=${encodeURIComponent(file.id)}`}
                    className={[
